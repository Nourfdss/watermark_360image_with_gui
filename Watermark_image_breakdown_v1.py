...
# opening image
def watermarking(img_path):

    OUTPUT_IMAGE_FILE = img_path + ".watermark.png"
    FONT_LOCATION = "calibrib.ttf"
    FONT_SIZE = 80
    H_SPACING = 70
    V_SPACING = 90
...
image_edit_values = [
    [sg.Text("Font opacity      "), sg.InputText(size=(20,10))], [sg.Text("Font Size          "), sg.InputText(size=20)],
    [sg.Text("Water Mark Text"), sg.InputText(size=20)]
]

layout = [
    [
        #sg.Text("Hello this watermark software")], [sg.Button("start")], [sg.Button("Exit")],
    sg.Column(file_list_column), sg.VSeperator(),sg.Column(image_edit_values) ,sg.VSeperator(),sg.Column(image_viewer_column),
        ...
        for filename in os.listdir(directory):
            if filename.endswith(".jpg") or filename.endswith(".png"):
               # path = os.path.join(directory, filename)
                ...
                watermarking(os.path.join(directory, filename))
                done = ["DONE"]
