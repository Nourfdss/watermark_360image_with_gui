...
# opening image
def watermarking(img_path, font_opacity, font_size, watermark_text):
    OUTPUT_IMAGE_FILE = img_path + ".watermark.jpg"
    FONT_LOCATION = "calibrib.ttf"
...
image_edit_values = [
    [sg.Text("Font opacity      "), sg.InputText(size=(20, 10), key="font_opacity", default_text=1)], [sg.Text("Font Size          ")
        , sg.InputText(size=20, key="font_size", default_text=80)],
    [sg.Text("Water Mark Text"), sg.InputText(size=20, key="watermark_text", default_text='TNB @ 2020')]
]
    ...
    if event == "start":  # or event == sg.WIN_CLOSED:
        ...
        for filename in os.listdir(directory):
            if filename.endswith(".jpg") or filename.endswith(".png"):
                ...
                watermarking(os.path.join(directory, filename), value_font_opacity, value_font_size, value_watermark_text)
                done = ["DONE"]
